...
function syJsbm7Chat(member_truename, head_image) {
    syJsbGetAppInfo(function (appInfo){
        if(!appInfo){
            return false;
        }
 ...
 * 检查app定位权限
 */
function syJsbCheckLocationPermission(){
  syJsbGetAppInfo(function (appInfo){
    if(!appInfo){
      return false;
    }
    var params = { method: 'checkLocationPermission', params: {} };
    var jsonParam = JSON.stringify(params);
    var platform=appInfo.split("|")[1];
    var version=appInfo.split("|")[5];
    if(platform=='ios'){
 ...
 * 获取app位置信息
 */
function syJsbLocationInfo(callback){
  syJsbGetAppInfo(function (appInfo){
    if(!appInfo){
      return false;
    }
    var params = { method: 'getLocationInfo', params: {} ,callback: callback };
    var jsonParam = JSON.stringify(params);
    var platform=appInfo.split("|")[1];
    var version=appInfo.split("|")[5];
    if(platform=='ios'){
      if (version > 88){
        syJsbCallApp(jsonParam);
        setupWebViewJavascriptBridge(function(bridge) {
          bridge.registerHandler(callback, function(json_data) {
            var data = eval('(' + json_data + ')');
            var json_data = { info: data.info, provincial: data.provincial ,city: data.city,county:data.county,longitude:data.longitude,latitude:data.latitude };
