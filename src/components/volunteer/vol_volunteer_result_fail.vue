  ...
  <div>
    ...
    <div class="wrapper">
      ...
      <div class="volcontent">
        <div>
          <span class="span1">志愿组织名称：</span>
          <span class="span2">{{volunteer_org_name}}</span>
        </div>
        <div>
...
<script>
  export default{
    ...
    methods:{
      getData(){
        let volunteer_org_id = this.$route.query.volunteer_org_id
        this.$http.get(this.siteInfo.host+'/api/index.php?act=volunteer&op=get_my_refused_volunteer&volunteer_org_id='+volunteer_org_id).then(res=>{
          let a = res.data.content
          if(res.data.code == 200){
            this.volunteer_org_name = a.volunteer_org_name
            this.apply_time = a.apply_time
