  ...
  <div class="indie-leaderboard">
    ...
    <transition name="fade">
      <div v-show="!isLoading" class="indie-leaderboard-con">
        <!--员工累计注册人数-->
        <div class="actives" v-if="memberList.length">
          <div class="achiTit" @click="_appjs.syJsbLaunchWebview(registerGoUrl)">
          ...
          <div class="activeContent">
            <div class="activeCon">
              <div class="for-li" v-for="(item,index) in memberList">
                ...
                <!--progress-->
                <!--<div class="redLine">
                  <p v-if="Number(memberList[0].count_num)"
                     class="p progress" :class="{'bg-light': index>=3}" :index='index'
                     :style="{width:100*item.count_num/memberList[0].count_num+'%'}"></p>
                  <p v-if="!Number(memberList[0].count_num)"
                     class="p progress" :class="{'bg-light': index>=3}" :index='index'
                     style="width: 0%"></p>
                </div>-->
...
<script>
  ...
  export default{
    ...
    created (){
      ...
      this.registerGoUrl = this.siteInfo.host +'/bocm/index.php?act=union&op=special_dept_member_rank_list&circle_id=' + this.circleId;
      this.consumptionGoUrl = this.siteInfo.host + '/bocm/index.php?act=union&op=special_dept_order_avg_rank_list&circle_id='+ this.circleId;
      this.integralGoUrl = this.siteInfo.host + '/bocm/index.php?act=union&op=special_dept_points_rank_list&circle_id=' + this.circleId;
      this.requestDate();
...
//          员工累计注册
            this.memberList = content.member_list.slice(0,10);
//          人均消费
...
//          积分排名
      ...
      shareFn(){
        if(this.web_share == 'yes' || !this.isApp){
          ...
          let shareUrl = this.siteInfo.host_share + '/bocm/index.php?act=union&op=special_dept_rank_info&circle_id=' + this.circleId + '&web_share=yes';
