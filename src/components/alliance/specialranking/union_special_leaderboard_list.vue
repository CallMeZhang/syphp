  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        let url = this.siteInfo.host + '/api/index.php?act=union&op=get_special_list&union_id=' + this.union_id + '&list=' + this.list + '&page=' + this.curpage
        this.$http.get(url).then((res) => {
          if (res.data.code === 200 && res.data.content.length !== 0) {
            ...
            let curTitle = this.name.replace(/公益/i,'');
            //      显示标题
            let currentPageTitle = "";
            if(this.list === 'member'){
              currentPageTitle = '注册人数光荣榜';
            }else if(this.list === 'order_avg'){
              currentPageTitle = '人均消费光荣榜';
            }
            document.title = currentPageTitle;
            this._appjs.syJsbSetTitleText(currentPageTitle);
            if (this.list === 'member') {
