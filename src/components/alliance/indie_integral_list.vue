    ...
    <transition name="fade">
      <div v-show="!isLoading" class="indie-integral-con">
        ...
        <div class="ranking-list" v-for="(list, index) in integralLists" :key="index" @click="valIndex=-1;showToast(index)">
          <div class="left-con"><span
            :class="[{top1:list.member_rank == 1,top2:list.member_rank == 2,top3:list.member_rank == 3}]"></span><img
            v-lazy="list.member_avatar" alt="photo"></div>
          <div class="right-con">
...
<script>
  ...
  export default{
    ...
    created (){
      this.union_name = this.$route.query.union_name || '';
      this.union_code = this.$route.query.code || '';
      this.getData();
    },
    methods: {
      getData(){
        let url = this.siteInfo.host + '/api/index.php?act=union&op=get_union_all_rank_list&curpage=' + this.curPage + '&code=' + this.union_code;
        this.$http.get(url).then((res) => {
          this.isLoading = false;
          if (res && res.data.code == 200) {
            this.integralLists = this.integralLists.concat(res.data.content);
            /*let curTitle = this.union_name;
            curTitle = curTitle.replace(/公益/i,'');
            document.title = curTitle  + '个人扶贫光荣榜';
            this._appjs.syJsbSetTitleText(curTitle  + '个人扶贫光荣榜');*/
            this.curPage++;
